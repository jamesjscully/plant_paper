...
plot3(Cavar_av*1.01,0*xvar_av,xvar_av+0.01,'Color',[.0 .0 .0 ],'LineWidth',2)
hold on
...
Vvar= x(1,1:end);
 ...
 hold on
 end2=399;
  plot3 (Cavar(end1:end2),Vvar(end1:end2),xvar(end1:end2),'.','Color',[0.5 0.5 0.5],'LineWidth',1.8)
 hold on
...
Vvar= x(1,1:end);
 ...
 Cavar=x(6,1:end);
 end1=460;
 plot3 (Cavar(1:end1),Vvar(1:end1),xvar(1:end1),'Color',[0.6 0.6 0.6],'LineWidth',1.5)
 hold on
 

 clear all 
load('caprime_n25.mat')
% %  % 
Vvar= x(1,1:end);
 nvar= x(2,1:end); 
 xvar= x(5,1:end);
 Cavar=x(6,1:end);
 end1=460;
 plot3 (1.3*Cavar(1:end1),Vvar(1:end1),1.35*xvar(1:end1)-0.04,'--','Color',[0.99 0.8 0.9],'LineWidth',2)
 hold on 
...
st=30090;
end1=45040;
plot3(Caa1(st:end1),vv1(st:end1),xx1(st:end1),'Color',[.9 .1 .1],'LineWidth',2)
hold on

st=  45200;
end1=45000;
plot3(Caa1(st:end),vv1(st:end),xx1(st:end),'Color',[.0 .2 .8],'LineWidth',2)
hold on
...
s3=surf (mag0*X+x3, mag0*Y+y3-1, mag0*Z/1.5+z3,'EdgeColor','none','FaceColor',...
    [0./255  51./255  225./255],'FaceAlpha',0.99) ;
hold on
...
%------------------- plot SNIC ---------------------
plot3(Ca_cod_bkgh0, 0*Ca_cod_bkgh0,x_cod_bkgh0,'-.','Color',[.85 .85 .85 ],'LineWidth',4)
hold on
plot3(Ca_cod_fdgh0, 0*Ca_cod_fdgh0,x_cod_fdgh0,'-.','Color',[.85 .85 .85 ],'LineWidth',4)
hold on
...
%txpo = text(1.02,0.5,0.76,'TS','Fontsize',17,'Color','blue','FontName','Arial')
txpo = text(0.54,0.5,0.875,'\langle x/V \rangle ','Fontsize',fftt,'Color','black','FontName','Arial')
txpo = text(1.31,0.5,0.89,'SNIC','Fontsize',fftt,'Color',[0.6 0.6 0.6],'FontName','Arial')
txpo = text(0.5,0.5,0.42,'[Ca]^\prime=0','Fontsize',fftt,'Color','black','FontName','Arial')

txpo = text(1.3,0.5,0.73,'x^\prime=0','Fontsize',fftt,'Color','black','FontName','Arial')
txpo = text(0.9,0.5,0.5,'x_{syn1}^\prime=0','Fontsize',fftt,'Color','black','FontName','Arial')
txpo = text(0.95,0.5,0.3,'x_{syn2}^\prime=0','Fontsize',fftt,'Color','black','FontName','Arial')
...
%print(gcf,'-djpeg','-r600' ,'nemoCaX_Q_after_pulse.jpeg');
...
load('TS_Q_after_pulse1.mat')
 ...
 subplot(2,1,1)
 plot(time-25, vv1,'Color',[0 0.2 0.8],'LineWidth',2);
 hold on
 st=30120;
end1=35040;
 plot(time(st:end1)-25, vv1(st:end1),'Color',[0.9 0.1 0.1],'LineWidth',2);
 hold on
 xlim([25-25 55-25]) 
 ylim([-65 45])
...
%print(gcf,'-djpeg','-r600' ,'V_Q_after_pulse.jpeg');
...
s3=zeros(length(vv2),1);
s=0.02;
s1=0.01;

for i=2:1:length(vv1)
s=s+.01*(alpha*s*(1-s)/(1+exp(-5*(vv1(i)+40)))-beta*s*(1-0.00001));
%s1=s1+1*(alpha*s1*(1-s1)*s-beta*s1*(1-0.001));
s2(i)=s;
%s3(i)=s1;
end
...
plot(time-25, s2,'Color',[0.1 0.2 0.8],'LineWidth',2);
 ...
 %plot(time-25,smoothdata(smoothdata(s2)),'Color',[0.9 0.9 0.99],'LineWidth',2);
 hold on
