...
tmax=20600; % max time in seconds
...
% Initial values
V1= -44; V2= 20; Ca1=.2; Ca2=0.92; h1 =0; h2 =0; n1 =0; n2 =0; 
x1 =0.2;x2 =.65; y1 =0; y2 =0; s1 =0; s2 =0; m1 =0; m2 =0;
...
%  
 time=time/1000;
[v1min,tp1min] = findpeaks(-vv1,time,'MinPeakProminence',1);
...
clf
 ...
 hold on
 plot(tp1min,-v1min,'.','Color',[1 0   0]','MarkerSize',20)
 hold on
 ...
 ylim([-75 40])
  fftt = 28;
txpo = text(-6.2,25,'B','Fontsize',fftt,'Color','black','FontName','Arial','FontWeight','bold')
 ...
 hold on
[Ca1min,tp1min] = findpeaks(-Caa1,time,'MinPeakProminence',.1);
 plot(tp1min,-Ca1min,'.','Color',[1 0   0]','MarkerSize',20)
 hold on
 ...
 hold on
[x1min,tp1min] = findpeaks(-xx1,time);
 plot(tp1min,-x1min,'.','Color',[1 0   0]','MarkerSize',20)
 hold on
 ...
 box on 
  ...
  figure(5)
  clf
  plot(Caa1,xx1,'Color',[0. 0.0 0.8],'Linewidth',2.0)
  hold on
  plot(Caa2,xx2,'Color',[0. 0.7 0.],'Linewidth',2.)
  hold on
...
figure(7)
clf
plot (-v1min1,-v1min,'.','Markersize',10)
hold on
...
figure(8)
clf
plot (-Ca1min1,-Ca1min,'.','Markersize',10)
hold on

  x1min1=circshift(x1min,1);
figure(9)
clf
plot (-x1min1,-x1min,'.','Markersize',10)
hold on

 % print(gcf,'-djpeg','-r600' ,'HCO1.jpeg');
