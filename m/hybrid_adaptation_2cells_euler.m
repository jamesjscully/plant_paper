...
V1 =V1 +step*(Iapp*heaviside(tt-t1)*heaviside(t2-tt)+4*((0.1*(50-(127*V1/105+8265/105))/(exp((50 - (127*V1/105 + 8265/105))/10) - 1))/((0.1*(50 - (127*V1/105 + 8265/105))/(exp((50 - (127*V1/105 + 8265/105))/10) - 1))+(4*exp((25 - (127*V1/105 + 8265/105))/18))))^3*h1*(30 - V1) + 0.3*n1^4*(-75 - V1)+0.01*x1*(30-V1) +0.03*Ca1/(.5 + Ca1)*(-75 - V1)+0.003*(-40 - V1)   +gh*((1/(1+exp(-(V1+63)/7.8)))^3)*y1*(+120-V1)-g21*(V1+55)*s2*m2+gelec*(V2-V1)+noise*(rand-0.5));
V2= V2 +step*(0*Iapp*heaviside(tt-t1)*heaviside(t2-tt)+4*((0.1*(50-(127*V2/105+8265/105))/(exp((50 - (127*V2/105 + 8265/105))/10) - 1))/((0.1*(50 - (127*V2/105 + 8265/105))/(exp((50 - (127*V2/105 + 8265/105))/10) - 1))+(4*exp((25 - (127*V2/105 + 8265/105))/18))))^3*h2*(30 - V2) + 0.3*n2^4*(-75 - V2)+0.01*x2*(30-V2) +0.03*Ca2/(.5 + Ca2)*(-75 - V2)+0.003*(-40 - V2) +gh*((1/(1+exp(-(V2+63)/7.8)))^3)*y2*(-V2+120)-g12*(V2-55)*s1*m1+gelec*(V1-V2)+noise*(rand-0.5));
Ca1=Ca1+step*(0.00003*(0.0085*x1*(140-V1+Ca_shift1)-Ca1));
Ca2=Ca2+step*(0.00003*(0.0085*x2*(140-V2+Ca_shift2)-Ca2));
x1 =x1+step*(((1/(exp(0.15*(-V1-50+x_shift))+1))-x1)/400);
x2 =x2+step*(((1/(exp(0.15*(-V2-50+x_shift))+1))-x2)/400);
...
%     
 ...
 ylim([-65 45])
 subplot(4,1,2)
 plot(time,-vv2.*vv1,'Color',[0 1 1]','LineWidth',1.)
 hold on
