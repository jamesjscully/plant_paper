...
% Initial values
V1= -65; V2= -65; Ca1=.6; Ca2=0.921; h1 =0; h2 =0; n1 =0; n2 =0; 
x1 =0.85;x2 =.85; y1 =0; y2 =0; s1 =0; s2 =0; m1 =0; m2 =0;
...
%  
 time=time/1000;
[v1min,tp1min] = findpeaks(-vv1,time,'MinPeakProminence',1,'MinPeakHeight',53.5);
[v2min,tp2min] = findpeaks(-vv2,time,'MinPeakProminence',1,'MinPeakHeight',53.5);
[v1max,tp1max] = findpeaks(vv1,time,'MinPeakHeight',20,'MinPeakDistance',0);
...
clf
 ...
 hold on
 plot(tp1max,v1max,'.','Color',[0 1   0]','MarkerSize',10)
   hold on
...
txpo = text(-82.2,25,'B','Fontsize',fftt,'Color','black','FontName','Arial','FontWeight','bold')
 ...
 hold on
[Ca1min,tp1minCa] = findpeaks(-Caa1,time,'MinPeakProminence',.01);
[Ca2min,tp2minCa] = findpeaks(-Caa2,time,'MinPeakProminence',.01);
 plot(tp1minCa,-Ca1min,'.','Color',[1 0   0]','MarkerSize',10)
 hold on
 ...
 hold on
[x1min,tp1minx] = findpeaks(-xx1,time,'MinPeakHeight',-0.6);
[x2min,tp2minx] = findpeaks(-xx2,time,'MinPeakHeight',-0.6);
 plot(tp1minx,-x1min,'.','Color',[1 0   0]','MarkerSize',10)
 hold on
 ...
 box on 
  ...
  j=0;
  for i=1:length(vv1)-1
  if  (xx1(i)<=0.6) && (xx1(i+1)>0.6)
  j=j+1;
  ic(j)=i;
  end
  end
 ...
 %size(ic)
  figure(5)
  clf
  plot(Caa1,xx1,'Color',[0. 0.0 0.8],'Linewidth',0.5)
  hold on
  plot(Caa1(ic),xx1(ic),'.','Color',[1. 0.0 0.0],'MarkerSize',10.0)
  hold on
...
%   figure(11)
%   clf
%   %plot3(Caa2,xx2,vv2,'Color',[0.2 0.9 0.0],'Linewidth',.5)
%   hold on
%   plot3(Caa1,xx1,vv1,'Color',[0. 0.0 0.8],'Linewidth',.5)
%   hold on
%   plot3(Caa1(ic),xx1(ic),vv1(ic),'.','Color',[1. 0.0 0.0],'MarkerSize',10.0)
%   hold on
% %   tran=8000;
% %   plot3(Caa1(1:tran),xx1(1:tran),vv1(1:tran),'Color',[0.99 0.0 0.0],'Linewidth',3.0)
% %   hold on
% %   pos=1;
% %   plot3(Caa1(pos),xx1(pos),vv1(pos),'.','Color',[1. 0.0 0],'MarkerSize',30)
% %   view(76,12)
  
% figure(12)
% clf 
% Caa11=circshift(Caa1(ic),1);
% plot (Caa11,Caa1(ic),'.','Color',[0.2 0.2 0.2])
...
figure(17)
clf
taun=tp1minCa-circshift(tp1minCa,1);
taun1=circshift(taun,1);
plot (taun1(10:end),taun(10:end),'.','Color','r','Markersize',10)
hold on  
% % clear taun
% taun=tp1minx-circshift(tp1minx,1);
% taun1=circshift(taun,1);
% plot (taun1(10:end),taun(10:end),'.','Color','k','Markersize',10)
% hold on  

for i =5:50%length(Ca1min)-2
plot ([taun1(i),taun1(i)],[taun1(i),taun(i)],'Color',[0.5 0.5 0.5],'Linewidth',1)
hold on
plot ([taun1(i),taun(i)],[taun(i),taun(i)],'Color',[0.5 0.5 0.5],'Linewidth',1)
hold on
end
plot ([0,11],[0,11],'k')
hold on
...
% 
% for i=1:length(tp1minCa)-1;
% ttau(i)=tp1minCa(i+1)-tp1minCa(i)
% end
% for i=1:length(ttau)-1
% plot (ttau(i),ttau(i+1),'.','Color','k','Markersize',10)
% hold on 
% end

figure(18)
clf
taunmax=tp1max-circshift(tp1max,1);
taunmax1=circshift(taunmax,1);

% % 
% clear ttau 
%  for i=1:length(tp1max)-1;
%  ttau(i)=tp1max(i+1)-tp1max(i);
%  end
%  for i=1:length(ttau)-1
%  plot (ttau(i),ttau(i+1),'.','Color','k','Markersize',10)
%  hold on 
%  end
% taunmin=tp1min-circshift(tp1min,1);
% taunmin1=circshift(taunmin,1);
% plot (taunmin1(10:end)/2,taunmin(10:end),'.','Color','r','Markersize',10)
% hold on


for i =5:50%length(Ca1min)-2
plot ([taunmax1(i),taunmax1(i)],[taunmax1(i),taunmax(i)],'Color',[0.7 0.7 0.7],'Linewidth',.1)
hold on
plot ([taunmax1(i),taunmax(i)],[taunmax(i),taunmax(i)],'Color',[0.7 0.7 0.7],'Linewidth',.1)
hold on
end
plot ([0,11],[0,11],'k')
hold on
plot (taunmax1(3:end),taunmax(3:end),'.','Color','g','Markersize',10)
hold on 
axis ([0 10.5 0 10.5])
...
for i =5:20%length(Ca1min)-2
plot ([-Ca1min1(i),-Ca1min1(i)],[-Ca1min1(i),-Ca1min(i)],'Color',[0.2 0.2 0.2],'Linewidth',1)
hold on
...
% hold on
% plot (-x1min1,-x1min,'.','Color','b','Markersize',10)
% hold on
