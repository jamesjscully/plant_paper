...
-g21*(V1+80)*s2*s2+gelec*(V2-V1)+rnd1(i));
...
s1 =s1+step*(alpha1*(1-s1)/(1+exp(-20*(V1+30)))-beta1*(s1-0.000));
s2 =s2+step*(alpha2*(1-s2)/(1+exp(-20*(V2+10)))-beta2*(s2-0.000));
...
%  hold on
 ...
 % plot(time,10*mm1.*ss1,'blue')
  ...
  hold on
  plot(time,ss2.*ss2,'Color',[0.1 0.8 0.1],'LineWidth',1.5)
  hold on
  %ylim([0 .02])
  xlim([0 time(end)])
 ...
 subplot(4,1,3)
 plot(time,nn2,'Color',[0.0 1 0])
 hold on
 xlim([0 6])
 
subplot(4,1,4)
   plot(time,yy2,'Color',[0 .2 .4])
   hold on
  xlim([0 6]) 
%   
