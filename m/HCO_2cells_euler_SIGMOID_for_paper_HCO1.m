...
V1= -44; V2= 20;  Ca2=1.1; h1 =0; h2 =0; n1 =0; n2 =0; x1 =0.25;
x2 =.9; y1 =0; y2 =0; s1 =0; s2 =0; m1 =0; m2 =0;
...
s1 =s1+step*(alpha*s1*(1-s1)/(1+exp(-10*(V1+20)))-beta*(s1-0.0001));
s2 =s2+step*(alpha*s2*(1-s2)/(1+exp(-10*(V2+20)))-beta*(s2-0.0001));
...
clf
 ...
 ylim([-85 40])
  ...
  fftt = 28;
txpo = text(-5.9,25,'B','Fontsize',fftt,'Color','black','FontName','Arial','FontWeight','bold')
 ...
 box on 
  ...
  ylim([-.1 1.])
  xlim([0 tmax])
  xlabel('Time [sec]','Fontsize', 14),ylabel('rate S(t)','Fontsize', 16)
 ...
 xlabel('Time [sec]','Fontsize', 14),ylabel('[Ca]','Fontsize', 16)
  %title('Ca and Ca-activaterd possasion gating varaible in time', 'Fontsize', 11);
   ...
   print(gcf,'-djpeg','-r600' ,'HCO7a.jpeg');
  
  toc 
 
  figure(5)
  clf
  plot(Caa1,xx1,'Color',[0. 0.0 0.8],'Linewidth',2)
  hold on
  plot(Caa2,xx2,'Color',[0.0 0.7 0.0],'Linewidth',2)
  hold on
  xlabel('[Ca]-variable','Fontsize', 14),
  ylabel('x-variable','Fontsize', 14)  
...
plot(Caa1(pos),xx1(pos),'.','Color',[0.5 0.7 1],'MarkerSize',40)
  ...
  hold on
  plot(Caa2(end),xx2(end),'.','Color',[0. 0.4 0.],'MarkerSize',20)
  hold on 
...
axis([0 1.5 0 1.0])
 ...
 print(gcf,'-djpeg','-r600' ,'HCO7.jpeg');
