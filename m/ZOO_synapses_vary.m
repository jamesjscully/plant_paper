...
for i =1:min(length(vp),length(vp1))
 ...
 hold on
 plot(t,1.6*s(:,8),'Color',[.5 .5 .5],'LineWidth',2)
 hold on
  plot(tav,1.6*avs,'-','Color',[1 1 1],'LineWidth',2.5)
  hold on
   plot(tav,1.6*avs3,'--','Color',[1 1 1],'LineWidth',2)
  hold on
 ylabel('\alpha-synapse','Fontsize',16)
 ylim([-0.01 1]); ; xlim([0 tmax])   
...
for i =1:min(length(vp),length(vp1))
 ...
 axes(ha(4));
 plot(t,5*s(:,7).*s(:,10),'Color',[.1 .7 .7],'LineWidth',2)
 hold on
%  plot(tp1,5*vp1,'x')
%   hold on
%  plot(tp,-15*vp)
%   hold on
 plot(tav1,5*avs1,'-','Color',[1 1 1],'LineWidth',2)
  hold on
...
for i =1:min(length(vp),length(vp1))
 ...
 axes(ha(5));
 plot(t,1.1*s(:,11),'red','LineWidth',2)
   hold on
% %plot(tp1,1.4*vp1)
%   hold on
% %plot(tp,-1.4*vp)
%   hold on
 plot(tav2,1.1*avs2,'-','Color',[1 1 1],'LineWidth',2)
   hold on  
%   
   ax = gca;
 ax.FontSize = 14;
...
print(gcf,'-djpeg','-r600' ,'Zoo_synapses_vаry.jpeg');
...
qd(10)  =(.0+1.0/(1+exp(-1000*(V1+10)))-0.8*m1)/4500;
