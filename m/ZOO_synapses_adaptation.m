...
% Initial Conditions
%      V Ca  h  n  x y s r m   
s0 = [-43.85 1.2 0 0 1 0 0 0 0 0 0 -35 1.5 0. 0 1 0 0.0];
...
for i =1:min(length(vp),length(vp1))
 ...
 axes(ha(4));
 plot(t,9*s(:,7).*s(:,10),'Color',[.1 .7 .7],'LineWidth',1.5)
 hold on
...
%  plot(tp,-15*vp)
%   hold on
 plot(tav1,9*avs1,'-','Color',[1 1 1],'LineWidth',1.5)
  hold on
...
for i =1:min(length(vp),length(vp1))
 ...
 axes(ha(5));
 plot(t,1.4*s(:,11),'red','LineWidth',1.5)
   hold on
% %plot(tp1,1.4*vp1)
%   hold on
% %plot(tp,-1.4*vp)
%   hold on
 plot(tav2,1.4*avs2,'-','Color',[1 1 1],'LineWidth',1.5)
   hold on  
%   
   ax = gca;
 ax.FontSize = 14;
...
qd(11) =2.5*alpha*s3*(1-s3)/(1+exp(-1000*(V1+40)))-.006*(s3-0.001);
...
qd(13) =0.00003*(0.0085*x2*(140 - V2+20)-Ca2);
