...
% Initial values
V1= -44; V2= -44; Ca1=.4; Ca2=1.3; h1 =0; h2 =0; n1 =0; n2 =0; 
x1 =0.5; x2 =.4; y1 =0; y2 =0; s1 =0; s2 =0; m1 =0; m2 =0;
...
%  hold on
 ...
 ylim([-85 40])
  fftt = 28;
txpo = text(-5.7,25,'B','Fontsize',fftt,'Color','black','FontName','Arial','FontWeight','bold')
 ...
 % plot(time,10*mm1.*ss1,'blue')
  ...
  hold on
  plot(time,3*ss2/scale2,'Color',[0.1 0.6 0.1],'LineWidth',1.5)
  hold on
  ylim([-0.001  .6])
  xlim([0 tmax1])
  box on
...
subplot(4,1,4)
 ...
 %print(gcf,'-djpeg','-r600' ,'fredo_TSb.jpeg');
...
%------------------- plot SNIC ---------------------
plot(Ca_cod_bkgh0,x_cod_bkgh0,'-.','Color',[.1 .1 .5 ],'LineWidth',1)
hold on
plot(Ca_cod_fdgh0,x_cod_fdgh0,'-.','Color',[.1 .1 .5 ],'LineWidth',1)
hold on


plot(Ca_cod_bkgh0,x_cod_bkgh0-0.02,'-.','Color',[.1 .5 .1 ],'LineWidth',1)
hold on
plot(Ca_cod_fdgh0,x_cod_fdgh0-0.02,'-.','Color',[.1 .5 .1 ],'LineWidth',1)
hold on


axis([0.01 1.5 0.01 1.02])

fftt = 28;
txpo = text(-.12,.98,'А','Fontsize',fftt,'Color','black','FontName','Arial','FontWeight','bold')

fftt = 15;
txpo = text(0.18,.13,'[Ca]^\prime=0','Fontsize',fftt,'Color','red','FontName','Arial')

fftt = 15;
%txpo = text(.8,.58,'[Ca]^\prime=0','Fontsize',fftt,'Color','red','FontName','Arial')


fftt = 16;
txpo = text(1.35,.15,'x^\prime=0','Fontsize',fftt,'Color','black','FontName','Arial')

fftt = 16;
txpo = text(.02,.23,'SNIC','Fontsize',fftt,'Color',[0.6 0.6 0.6],'FontName','Arial')
...
%all currents  
Q1 =-gsyn*(V1-20) -Iapp -INa  -IK -Ittx -Ileak -Ihh;
 
% solve for Q1 = -0.03*Ca1./(Ca1+0.5).*(V1+75)
Ca2=0.5*Q1./(0.03*(V1-VK)-Q1);

%x22=Ca1./(0.0085*(140-V1+dca));
plot (Ca2,x1,'Color','[0 .99 0]','LineWidth',1)
hold on

% nullcline Ca'=0
Q11=-gsyn*(V1-20)-Iapp+gI*minf.^3.*h1.*(VI-V1) + gK*n1.^4.*(VK-V1) +0.03*Ca1.*(V1+75)./(0.5+Ca1) -gL*(V1-VL) -gh*(((1./(1.+exp(-(V1+63.)/7.8))).^3).*y1.*(V1-70));
x11=Q11./(gT*(VI-V1));
...
%x22=Ca1./(0.0085*(140-V1+dca));
plot (Ca2,x1,'Color',[0 0 0.99],'LineWidth',1)
hold on
...
%print(gcf,'-djpeg','-r600' ,'fredo_TSa.jpeg');


figure(6)
 clf
 st=1;
 plot(Caa1(st:end),Caa2(st:end),'Color',[0.1 0.2 0.2],'LineWidth',1.5)
 hold on
xlabel('[Ca]_1-variable','Fontsize', 16),
  ylabel('[Ca]_2-variable','Fontsize', 16)  
    fftt = 28;
txpo = text(0.05,1.74,'С','Fontsize',fftt,'Color','black','FontName','Arial','FontWeight','bold')

ylim([1.2  1.75])
  xlim([0.1 0.9])
  
 %print(gcf,'-djpeg','-r600' ,'fredo_Q1c.jpeg');