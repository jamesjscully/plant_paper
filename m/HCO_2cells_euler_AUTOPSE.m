...
% Initial values
V1= -44; V2= -44; Ca1=.0; Ca2=0.; h1 =0; h2 =0; n1 =0; n2 =0; x1 =1;
...
-g11*(V1+80)*s1*scale1+gelec*(V2-V1)+rnd1(i));
...
-g22*(V2+80)*s2*scale1+gelec*(V1-V2)+rnd2(i));
...
s1 =s1+step*(alpha1*s1*(1-s1)/(1+exp(-10*(V1+30)))-beta1*(s1-0.0000001));
s2 =s2+step*(alpha2*s2*(1-s2)/(1+exp(-10*(V2+30)))-beta2*(s2-0.0000001));
...
%  hold on
 ...
 % plot(time,10*mm1.*ss1,'blue')
  hold on
  plot(time,ss1*scale1,'Color',[0.1 0.1 0.8],'LineWidth',1.5)
  plot(time,ss2*scale2,'Color',[0.1 0.8 0.1],'LineWidth',1.5)
  hold on
  %ylim([0  .01])
  xlim([0 time(end)])
 ...
 clf
 st=15000;
 plot(Caa1(st:end),xx1(st:end),'b','LineWidth',1.5)
  hold on
  plot(Caa2(st:end),xx2(st:end),'green','LineWidth',1.5)
  hold on
  ...
  clf 
   ...
   hold on
   title('s1(t) vs s2(t)', 'Fontsize', 11);