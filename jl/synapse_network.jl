...
begin
    ...
    for s in sp
        ...
        net = Network([c])
        n = generate(net())
        prob = ODEProblem(n.f, n.u0, (0.,5000.))
    ...
    vpre1_linear(s) = (varr[1]*(sp[end]-s) + varr[end]*(s-sp[1]))/(sp[end]-sp[1])
    ...
    vpre2_linear(s) = (varr2[1]*(sp[end]-s) + varr2[end]*(s-sp[1]))/(sp[end]-sp[1])
    u0 = [0.2,0.8]
    p = (
        α1 = α1,
        β1 = β1,
    ...
    function fs1(S1,S2)
        α1,β1,ϵ1,♉1,k1,α2,β2,ϵ2,♉2,k2 = p
        α1*S1*(1-S1)/(1+exp(♉1-vpre1(S2))/k1) - β1*(S1 -ϵ1)
    end
    function fs2(S1,S2)
        α1,β1,ϵ1,♉1,k1,α2,β2,ϵ2,♉2,k2 = p
        α2*S2*(1-S2)/(1+exp(♉2-vpre2(S1))/k2) - β2*(S2 -ϵ2)
    end
    rng = 0:.01:1
    contour!(ax,rng,rng,fs1, levels = [0])
    contour!(ax,rng,rng,fs2, levels = [0])
    f
end
